...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function themeViewComposers()
    {
        ...
        theme_compose(['workspace.items.includes.files-box'], function ($view) {
            $uploadedFiles = UploadedFile::where('author_id', Auth::user()->id)->notExpired()->get();
            $view->with('uploadedFiles', $uploadedFiles);
        });
    ...
    public function registerBladeDirectives()
    {
        ...
        Blade::directive('reviewerColors', function () {
            return '<link rel="stylesheet" href="' . asset(config('system.reviewer.colors')) . '">';
        });

        Blade::directive('reviewerCustomStyle', function () {
            return '<link rel="stylesheet" href="' . asset(config('system.reviewer.custom_css')) . '">';
        });
    }
}
